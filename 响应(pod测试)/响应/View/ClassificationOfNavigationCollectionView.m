//
//  ClassificationOfNavigationCollectionView.m
//  响应
//
...
@implementation ClassificationOfNavigationCollectionView

- (instancetype)initWithFrame:(CGRect)frame{
    
    if (self = [super initWithFrame:frame]) {
        [self addSubview:self.classificationOfNavigationView];
        [self layoutUI];
...
- (void)layoutUI{
    ...
    [self.classificationOfNavigationView mas_makeConstraints:^(MASConstraintMaker *make) {
...
- (NSInteger)collectionView:(UICollectionView *)collectionView numberOfItemsInSection:(NSInteger)section
{
    ...
    if ([self.selectedSectionArr[section]isEqualToString:@"0"]) {
        return 0;
    }else{
...
- (UICollectionViewCell *)collectionView:(UICollectionView *)collectionView cellForItemAtIndexPath:(NSIndexPath *)indexPath
{
    ...
    ClassificationOfNavigationCollectionCellCollectionViewCell *classificationOfNavigationCell = [collectionView dequeueReusableCellWithReuseIdentifier:@"collectionViewCell" forIndexPath:indexPath];
...
{
    return CGSizeMake(0, SIZEWIDTH_X(48, SCREEN_WIDTH));
}
...
//         CGFloat sizeHeight = SIZEWIDTH_X(48, SCREEN_WIDTH)*self.sectionCount;//基础高度
        ...
        __block CGFloat zongSizeHeight = SIZEWIDTH_X(48, SCREEN_WIDTH)+hangShu*SIZEWIDTH_X(48, SCREEN_WIDTH);
...
//
//        }];
//        [self.classificationOfNavigationView reloadData];
//        [self.classificationOfNavigationView reloadSections:[NSIndexSet indexSetWithIndex:sender.tag]];
...
//   __block int lieshu = 0;
//    for (int i=1 ; i<arrTitle.count+1; i++)
//    {
//        //        列数
//        lieshu=i%3;
//        NSLog(@"lieshu%d",lieshu);
//    }
    NSInteger hangShu = self.arrTitle.count/3;
    if (arrTitle.count%3!=0) {
        hangShu = hangShu+1;
...
//    if (self.selectedSectionArr.count==0&&self.sectionCount!=0) {
//
//    }
//    for (int i=0 ; i<self.sectionCount; i++){
//
//        [self.selectedSectionArr addObject:@"0"];
//    }
    __block CGFloat zongSizeHeight = SIZEWIDTH_X(48, SCREEN_WIDTH)+hangShu*SIZEWIDTH_X(48, SCREEN_WIDTH);
    zongSizeHeight = zongSizeHeight*self.sectionCount;
    [self.selectedSectionArr enumerateObjectsUsingBlock:^(id  _Nonnull obj, NSUInteger idx, BOOL * _Nonnull stop) {
...
//}
- (UICollectionView *)classificationOfNavigationView{
    
    if (!_classificationOfNavigationView) {
...
//                flowLayout.headerReferenceSize = CGSizeMake(0, 0);
        ...
        _classificationOfNavigationView = [[UICollectionView alloc]initWithFrame:CGRectZero collectionViewLayout:flowLayout];
        _classificationOfNavigationView.showsHorizontalScrollIndicator = NO;
        _classificationOfNavigationView.dataSource = self;
        _classificationOfNavigationView.delegate = self;
        _classificationOfNavigationView.backgroundColor = [UIColor whiteColor];
        
        [_classificationOfNavigationView registerClass:[ClassificationOfNavigationCollectionCellCollectionViewCell class] forCellWithReuseIdentifier:@"collectionViewCell"];
        
        [_classificationOfNavigationView registerClass:[PGroupSectionCollectionReusableView class] forSupplementaryViewOfKind:UICollectionElementKindSectionHeader withReuseIdentifier:@"GroupSectionID"];
        //        通用头部
//        [_classificationOfNavigationView registerNib:[UINib nibWithNibName:@"PGroupSectionCollectionReusableView" bundle:nil] forSupplementaryViewOfKind:UICollectionElementKindSectionHeader withReuseIdentifier:@"GroupSectionID"];
        
        
    }
    
    return _classificationOfNavigationView;
}
